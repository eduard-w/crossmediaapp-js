...
session.startPosition = new THREE.Vector3(0,1,2);

// background
new EXRLoader().load( './cma/examples/assets/photo_studio_broadway_hall_1k.exr', function ( texture ) {
...
session.raycastHelper.registerSingleObjectInWorld(chairContainer);
...
session.raycastHelper.registerObjectInWorld(floorMesh);
...
session.raycastHelper.registerObjectInGui(buttonPrev);
session.appMenu.add(buttonNext);
session.raycastHelper.registerObjectInGui(buttonNext);
session.appMenu.add(buttonRotateMode);
session.raycastHelper.registerObjectInGui(buttonRotateMode);
session.appMenu.add(buttonTranslateMode);
session.raycastHelper.registerObjectInGui(buttonTranslateMode);
session.appMenu.set({
	height: 1.3
});
...
session.addEventListener("started", (event) => {
	...
	if (session.mode == "mobile-ar" || session.mode == "hmd-ar") {
		...
		session.raycastHelper.unregisterObjectInWorld(floorMesh);
	...
	session.addEventListener("update", (event) => {
		if (chairSelected && interactionMode == "translate") {
			...
			if (newPosition == null) {
				console.error("newPosition is null");
			}
